...
export const useTaskStore = defineStore('task', () => {
  ...
  async function fetchRecentTasks(projectId, stagesData) {
    ...
    try {
      const res = await workflowApi.fetchTasks(projectId)
  ...
  async function toggleTaskDetail(taskId) {
    if (expandedTaskId.value === taskId) {
      ...
      if (!taskLogs.value[taskId]) {
        try {
          const res = await workflowApi.fetchTaskLogs(taskId)
  ...
  async function fetchActivityLogs(projectId) {
    ...
    try {
      const res = await workflowApi.fetchActivityLogs(projectId, logPage.value + 1)
