...
from core.executors.parsers.parser import parse_enw
...
class TaggedTextParserTests(TestCase):
    ...
    def test_blank_lines_inside_record_do_not_create_empty_references(self):
        entries = self._parse_bytes(TAGGED_TEXT.encode('utf-8'))

        self.assertEqual(len(entries), 2)
        self.assertEqual([item['title'] for item in entries], ['糖尿病筛查研究', '高血压干预研究'])
        self.assertEqual(entries[0]['authors'], ['张三', '李四'])
        self.assertEqual(entries[0]['journal'], '中华循证医学杂志')

    def test_common_chinese_encodings_are_supported_without_character_loss(self):
...
@override_settings(MEDIA_ROOT=tempfile.mkdtemp(prefix='txt-download-test-'))
class OriginalFileDownloadTests(TestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(username='txt-owner', password='test-pass')
        ...
        self.data_file = DataFile.objects.create(
            ...
            filename='中文索引.txt',
            file=SimpleUploadedFile('中文索引.txt', self.content, content_type='text/plain'),
            data_category='input',
    ...
    def test_other_user_cannot_download_project_file(self):
        other = get_user_model().objects.create_user(username='other-user', password='test-pass')
